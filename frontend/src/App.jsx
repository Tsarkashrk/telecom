...
import { authApi, exportInvoices, invoiceApi, subscriptionApi } from './api'
...
export default function App() {
  ...
  const displayedNav = isOperator ? [...navItems, { id: 'operator', label: 'Оператор' }] : navItems
  ...
  async function handleActivateTariff(tariffId) {
    ...
    try {
      ...
      showStatus('Тариф активирован в режиме предоплаты. Создан счет на оплату.', 'success')
    } catch (error) {
      showStatus(error.message, 'error')
    } finally {
      setIsBusy(false)
    ...
    return (
      <main className="shell auth-shell">
        <section className="auth-hero">
          ...
          <p>Интерфейс для защищенного MVP телеком-платформы: регистрация, аутентификация, активация тарифов, биллинг и аудит критичных действий.</p>

          <div className="hero-grid">
            <StatCard label="Роли" value="3" hint="customer, operator, admin" />
            <StatCard label="API" value="10+" hint="Авторизация, подписки, биллинг, export" />
            <StatCard label="Контроли" value="OWASP" hint="Валидация, RBAC, логирование, rotation" />
          </div>
        </section>
    ...
    <main className="shell app-shell">
      ...
      <section className="hero-panel">
        <div className="hero-copy">
          ...
          <p>Пользователь проходит безопасную аутентификацию, выбирает тариф, получает счет, оплачивает его и активирует подписку. Оператор и администратор могут просматривать клиентские данные в пределах своих прав.</p>
        </div>
        <div className="hero-grid">
          <StatCard label="Подписок" value={metrics.activeSubscriptions} hint="Активные и ожидающие оплаты" />
          <StatCard label="Неоплачено" value={metrics.unpaidInvoices} hint="Счета, требующие внимания" />
          <StatCard label="Сумма" value={formatMoney(metrics.totalDue)} hint="Текущая задолженность" />
        </div>
      </section>
      ...
      {activeTab === 'overview' ? (
        <section className="panel-grid two-columns">
          <article className="panel-card accent-card">
            ...
            <p>
              Аккаунт создан {formatDate(user.created_at)}. Текущая роль <strong>{user.role}</strong>, статус <strong>{user.is_active ? 'active' : 'inactive'}</strong>.
            </p>
          ...
          <article className="panel-card">
            ...
            <p>Если подписка еще не оформлена, перейдите в раздел тарифов. Если счет уже выпущен, оплатите его в биллинге для активации услуги.</p>
            <div className="quick-actions">
              <button className="primary-button" onClick={() => setActiveTab('tariffs')} type="button">
                Выбрать тариф
              </button>
              <button className="secondary-button" onClick={() => setActiveTab('billing')} type="button">
                Перейти к счетам
              </button>
            </div>
          </article>
          ...
          {tariffs.length ? (
            tariffs.map((tariff) => (
              <article className="tariff-card" key={tariff.id}>
                ...
                <button className="primary-button" disabled={isBusy} onClick={() => handleActivateTariff(tariff.id)} type="button">
                  Активировать
                </button>
              </article>
            ))
          ) : (
            <EmptyState title="Нет доступных тарифов" text="Backend не вернул активные тарифные планы." />
          )}
        </section>
      ) : null}
      ...
      {activeTab === 'subscriptions' ? (
        <section className="card-grid">
          {subscriptions.length ? (
            ...
            <EmptyState title="Подписок пока нет" text="Активируйте тариф, чтобы backend создал подписку и счет." />
          )}
        </section>
      ) : null}
      ...
      {activeTab === 'operator' ? (
        <section className="panel-stack">
          <article className="panel-card">
            ...
            <p>Введите ID пользователя, чтобы получить его подписки и счета.</p>
